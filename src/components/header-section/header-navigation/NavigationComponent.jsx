...
export default class HeaderNavigationComponent extends Component{
  ...
  componentDidMount = () => {
    ...
    this.setState({ navs: this.Subroutine.applicationNavigations() });    
  }
        ...
        <>
          ...
          <ul className="order-1 order-md-3 navbar-nav navbar-no-expand ml-auto">
            ...
            {/* START : PAGE NAVIGATION */}
              ...
              {this.state.navs.map( ( item, index ) => ( <li className="nav-item card-info card-outline-tabs card-outline" key={index}><LinkPageSection smooth to={item.endpoint} className={ (  index === 0 ? "nav-link active" : "nav-link" ) } data-toggle="pill">{item.endpointName}</LinkPageSection> </li> ) )}
